...
export function UserCardPopover({
  ...
  const isModerator = Boolean(moderatorStatus || target.flags.isModerator)
  const isSelf =
  ...
  const actorCanModerate =
    Boolean(account && channelRoomId && !isSelf && !target.flags.isBroadcaster) &&
    (actorIsBroadcaster || Boolean(selfChatState?.isModerator))
  const canUseBanApi =
    actorCanModerate &&
    hasUserCardScope(account, USER_CARD_MODERATION_SCOPES.ban)
  ...
  const bannerImageUrl = ivrProfile?.bannerImageUrl ?? ""
